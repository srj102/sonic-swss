...
def test_FDBReaddPortAddedBackToVlan(dvs, testlog):
    dvs.setup_db()
    ...
    dvs.runcmd("config mac add 52:54:00:25:06:E9 2 Ethernet0")
    ...
    time.sleep(2)
    # check that the FDB entry was flushed
    assert how_many_entries_exist(dvs.adb, "ASIC_STATE:SAI_OBJECT_TYPE_FDB_ENTRY") == 0, "The fdb entry was not flushed"
    dvs.create_vlan_member("2", "Ethernet0")
    time.sleep(2)
    # check that the FDB entry was inserted into ASIC DB
    assert how_many_entries_exist(dvs.adb, "ASIC_STATE:SAI_OBJECT_TYPE_FDB_ENTRY") != 0, "The fdb entry wasn't inserted to ASIC"
    dvs.runcmd("config mac del 52:54:00:25:06:E9 2")
    dvs.remove_vlan_member("2", "Ethernet0")
    dvs.remove_vlan("2")
